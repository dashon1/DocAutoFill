...
Deno.serve(async (req) => {
    ...
    const bucketConfig = {
        id: 'docautofill-documents',
        name: 'docautofill-documents',
        public: true
    };

    // Add optional configurations
    bucketConfig.allowed_mime_types = ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "image/*", "text/plain"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for docautofill-documents" ON storage.objects FOR SELECT USING (bucket_id = 'docautofill-documents');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for docautofill-documents" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'docautofill-documents');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for docautofill-documents" ON storage.objects FOR UPDATE USING (bucket_id = 'docautofill-documents');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for docautofill-documents" ON storage.objects FOR DELETE USING (bucket_id = 'docautofill-documents');`
    ];
    ...
    return new Response(JSON.stringify({
        ...
        name: 'docautofill-documents',
        public: true,
        allowed_mime_types: ["application/pdf", "application/msword", "application/vnd.openxmlformats-officedocument.wordprocessingml.document", "image/*", "text/plain"],
